...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    description:
      "Passive local network inspection with safe actions only: interface inventory, listener inventory, and bounded capture summaries. No privilege escalation or invasive scanning is performed.",
    parameters: Type.Object({
      action: Type.String({
        description:
          "Action to perform: interfaces, listeners, capture_summary",
      }),
      interface: Type.Optional(
        Type.String({
          description:
            "Interface name for capture_summary. Prefer loopback/authorized local interfaces only.",
        }),
      ),
      seconds: Type.Optional(
        Type.Number({
          description:
            "Bounded capture duration hint in seconds (default 3, max 10).",
        }),
      ),
      packet_count: Type.Optional(
        Type.Number({
          description: "Maximum packets to summarize (default 10, max 50).",
        }),
      ),
    }),
    async execute(_toolCallId, params) {
      ...
      const iface =
        typeof (params as any).interface === "string"
          ? (params as any).interface.trim()
          : "";
      const seconds = Math.max(
        1,
        Math.min(10, Number((params as any).seconds) || 3),
      );
      const packetCount = Math.max(
        1,
        Math.min(50, Number((params as any).packet_count) || 10),
      );
      ...
      try {
        ...
        if (action === "capture_summary") {
          if (!iface) {
            return {
              content: [
                {
                  type: "text" as const,
                  text: "capture_summary requires an interface name. Use the interfaces action first and prefer loopback or an explicitly authorized local interface.",
                },
              ],
          ...
          return {
            content: [
              {
                type: "text" as const,
                text: `Passive capture summary (${iface}, up to ${packetCount} packets):\n\n${output || "No packets captured within the bounded window."}`,
              },
            ],
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Unknown action: ${action}. Use interfaces, listeners, or capture_summary.`,
            },
          ],
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `network_inspect failed: ${error.message}`,
            },
          ],
    ...
    renderCall(args, theme) {
      const p = args as any;
      return new Text(
        theme.fg("toolTitle", theme.bold("network_inspect ")) +
    ...
    renderResult(result, _options, theme) {
      ...
      if (details?.error)
        return new Text(
          theme.fg("error", `network_inspect error: ${details.error}`),
          0,
          0,
        );
      return new Text(
        theme.fg("success", `network_inspect ${details?.action || "done"}`),
        0,
        0,
      );
    },
