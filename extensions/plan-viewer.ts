...
function startViewerServer(
  ...
  port: number;
  server: Server;
  waitForResult: () => Promise<ViewerResult>;
}> {
...
const ShowPlanParams = Type.Object({
  ...
  mode: Type.Optional(
    Type.String({
      description:
        "Viewer mode: 'plan' (default) for plan review/approval, or 'questions' for follow-up questions with inline answers",
    }),
  ),
});
...
export default function (pi: ExtensionAPI) {
  ...
  async function runViewer(
    ...
    // Start HTTP server
    const { port, server, waitForResult } = await startViewerServer(
      markdown,
      title,
      purpose,
    );
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      const purpose: ViewerPurpose =
        modeStr === "questions" ? "questions" : "plan";
