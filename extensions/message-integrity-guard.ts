  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (msg.role === "assistant") {
        ...
        for (const toolId of currentToolUseIds) {
          if (!satisfiedToolUseIds.has(toolId)) {
            ...
            const syntheticResult: ToolResultMessage = {
              ...
              content: [
                { type: "text", text: "[Result lost during session recovery]" },
              ],
              isError: true,
              timestamp: Date.now(),
            };
      ...
      if (currentToolUseIds.has(toolResult.toolCallId)) {
        ...
        const ownerAssistant = findAssistantWithToolUse(
          result,
        ...
        for (const toolId of currentToolUseIds) {
          if (!satisfiedToolUseIds.has(toolId)) {
            ...
            const syntheticResult: ToolResultMessage = {
              ...
              toolName: toolCall?.name ?? "unknown",
              content: [
                { type: "text", text: "[Result lost — user interrupted]" },
              ],
              isError: true,
              timestamp: Date.now(),
            };
    ...
    for (const toolId of currentToolUseIds) {
      if (!satisfiedToolUseIds.has(toolId)) {
        ...
        const syntheticResult: ToolResultMessage = {
          ...
          toolName: toolCall?.name ?? "unknown",
          content: [
            { type: "text", text: "[Result lost — end of recovered history]" },
          ],
          isError: true,
          timestamp: Date.now(),
        };
