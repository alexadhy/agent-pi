...
import {
  ...
  type ReportData,
  type ChangedFile,
} from "./lib/completion-report-html.ts";
import {
  createCompletionReportStandaloneExport,
...
function getFileStatuses(
  cwd: string,
  baseRef: string,
): Map<string, { status: ChangedFile["status"]; oldPath?: string }> {
  const statusMap = new Map<
    string,
    { status: ChangedFile["status"]; oldPath?: string }
  >();
...
function shouldSuppressReportFile(filePath: string): boolean {
  ...
  return (
    normalized.startsWith(".context/test-exports/") ||
...
function gatherReportData(
  ...
  for (const stat of stats) {
    const statusInfo = statuses.get(stat.path) || {
    ...
    files.push({
      ...
      diff: shouldSuppressReportFile(stat.path)
        ? summarizeSuppressedFile(stat.path)
        : diff,
...
function startReportServer(
  ...
  port: number;
  server: Server;
  waitForResult: () => Promise<ReportResult>;
}> {
            ...
            for (const filePath of files) {
              try {
                if (baseRef === "HEAD") {
                  ...
                  execSync(`git checkout ${baseRef} -- "${filePath}"`, {
                    cwd,
                    encoding: "utf-8",
                  });
                }
...
const ShowReportParams = Type.Object({
  title: Type.Optional(
    Type.String({
      description: "Title for the report (default: 'Completion Report')",
    }),
  ),
  summary: Type.Optional(
    Type.String({ description: "Markdown summary of the work done" }),
  ),
  base_ref: Type.Optional(
    Type.String({
      description:
        "Git ref to diff against (default: auto-detect — HEAD for uncommitted changes, HEAD~1 for committed)",
    }),
  ),
});
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (!isGitRepo(cwd)) {
        return {
          content: [
            {
              type: "text" as const,
              text: "Error: Not a git repository. The completion report requires git to gather file changes.",
            },
          ],
        };
      }
      ...
      if (report.files.length === 0) {
        return {
          content: [
            {
              type: "text" as const,
              text: "No file changes detected. Nothing to report.",
            },
          ],
        };
      }
      ...
      const { port, server, waitForResult } = await startReportServer(
        report,
        cwd,
      );
      activeServer = server;
