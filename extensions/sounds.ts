...
import {
  generateSoundsViewerHTML,
  type CatalogItem,
} from "./lib/sounds-viewer-html.ts";
import {
  loadConfig,
  saveConfig,
  getActiveAssignmentCount,
  getAssignedSoundNames,
  type SoundsConfig,
  type HookName,
  ALL_HOOKS,
  HOOK_DISPLAY_NAMES,
...
function startSoundsServer(
  ...
  port: number;
  server: Server;
  waitForResult: () => Promise<SoundsViewerResult>;
}> {
  return new Promise((resolveSetup) => {
    ...
    const server = createServer((req: IncomingMessage, res: ServerResponse) => {
      ...
      if (req.method === "GET" && url.pathname.startsWith("/api/sound/")) {
        const name = decodeURIComponent(
          url.pathname.slice("/api/sound/".length),
        );
        ...
        (async () => {
          try {
            const upstream = await fetch(
              `https://soundcn.xyz/r/${encodeURIComponent(name)}.json`,
            );
            if (!upstream.ok) {
      ...
      if (req.method === "POST" && url.pathname === "/install") {
        ...
        req.on("end", () => {
          try {
            ...
            res.writeHead(400, { "Content-Type": "application/json" });
            res.end(
              JSON.stringify({ error: err?.message || "Install failed" }),
            );
          }
        });
        return;
      ...
      if (req.method === "POST" && url.pathname === "/uninstall") {
        ...
        req.on("end", () => {
          try {
            ...
            res.writeHead(400, { "Content-Type": "application/json" });
            res.end(
              JSON.stringify({ error: err?.message || "Uninstall failed" }),
            );
          }
        });
        return;
    ...
    server.listen(0, "127.0.0.1", () => {
      ...
      resolveSetup({
        ...
        server,
        waitForResult: () =>
          resultPromise.finally(() => clearInterval(heartbeatCheck)),
      });
    });
  });
}
...
export default function (pi: ExtensionAPI) {
  ...
  async function runSoundsViewer(
    ctx: ExtensionContext,
  ): Promise<SoundsViewerResult> {
    cleanupServer();
    ...
    ctx.ui.notify(
      `Loaded ${catalog.length} sounds. Opening browser...`,
      "info",
    );

    // Start server
    const { port, server, waitForResult } = await startSoundsServer(
      catalog,
      currentConfig,
    );
    activeServer = server;
  ...
  pi.registerCommand("sounds", {
    description:
      "Open the sound browser, or use: /sounds toggle | /sounds status",
    handler: async (args, ctx) => {
      ...
      if (result.action === "applied") {
        const count = getActiveAssignmentCount(currentConfig);
        ctx.ui.notify(
          `✓ Sound config applied — ${count} hook${count !== 1 ? "s" : ""} assigned`,
          "info",
        );
      }
    },
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
      if (!ctx.hasUI) {
        return {
          content: [
            {
              type: "text" as const,
              text: "Sound browser requires interactive mode.",
            },
          ],
        };
      }
      ...
      if (result.action === "applied") {
        ...
        const assigned = Object.entries(currentConfig.assignments)
          .map(
            ([hook, sound]) =>
              `  ${HOOK_DISPLAY_NAMES[hook as HookName]}: ${sound}`,
          )
          .join("\n");
        return {
          content: [
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: "Sound browser closed without applying changes.",
          },
        ],
    ...
    renderCall(_args, theme) {
      const text =
        theme.fg("toolTitle", theme.bold("show_sounds ")) +
    ...
    renderResult(result, _options, theme) {
      ...
      if (details.action === "applied") {
        ...
        return new Text(
          outputLine(theme, "success", `Sound config applied — ${count} hooks`),
          0,
          0,
        );
      }
      return new Text(
        outputLine(theme, "warning", "Sound browser closed"),
        0,
        0,
      );
    },
