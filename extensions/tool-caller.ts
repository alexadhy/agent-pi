...
const CallToolParams = Type.Object({
  tool_name: Type.String({
    description:
      "Name of the tool to invoke (e.g. 'read', 'commander_task', 'web_remote')",
  }),
  ...
  reason: Type.Optional(
    Type.String({
      description:
        "Brief description of why this tool is being called (for audit trail)",
    }),
  ),
});
      ...
      if (BLOCKED_TOOLS.has(tool_name)) {
        return {
          content: [
            {
              type: "text" as const,
              text: `Error: Cannot call '${tool_name}' through call_tool — use it directly.`,
            },
          ],
      ...
      if (!entry) {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Error: Tool "${tool_name}" not found.${suggestion}`,
            },
          ],
      ...
      if (!toolDef) {
        return {
          content: [
            {
              type: "text" as const,
              text: `Error: Tool "${tool_name}" is indexed but not currently registered. It may have been unloaded.`,
            },
          ],
        ...
        if (executor) {
          ...
          return {
            content: result.content || [
              { type: "text" as const, text: "Tool returned no content" },
            ],
            details: {
              tool_name,
              reason,
          ...
          content: [
            {
              type: "text" as const,
              text:
                `Tool "${tool_name}" exists but programmatic execution is not available. ` +
  ...
  switch (name) {
    case "bash": {
      ...
      if (!command)
        return {
          content: [
            { type: "text", text: "Error: 'command' parameter required" },
          ],
        };
    ...
    case "read": {
      ...
      if (!path)
        return {
          content: [{ type: "text", text: "Error: 'path' parameter required" }],
        };
      try {
