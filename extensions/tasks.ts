...
export default function (pi: ExtensionAPI) {
  ...
  const refreshUI = (ctx: ExtensionContext) => {
    ...
    if (tasks.length === 0) {
      ...
      ctx.ui.setStatus(
        `${label}: ${tasks.length} tasks (${remaining} remaining) ${syncIndicator}`,
        "tasks",
      );
    }
  ...
  pi.registerTool({
    ...
    renderResult(result, { expanded }, theme) {
      ...
      if (details.error) {
        return new Text(
          outputLine(theme, "error", `Error: ${details.error}`),
          0,
          0,
        );
      }
      ...
      switch (details.action) {
        ...
        case "list": {
          if (taskList.length === 0)
            return new Text(outputLine(theme, "accent", "No tasks"), 0, 0);
        ...
        case "clear":
          return new Text(
            outputLine(theme, "success", "Cleared all tasks"),
            0,
            0,
          );
