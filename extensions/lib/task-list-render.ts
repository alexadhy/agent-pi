...
export function ensureTaskVisible(
  selectedIndex: number,
  scrollOffset: number,
  maxVisible: number = MAX_VISIBLE_TASKS,
): number {
...
export function computeHeightMode(
  taskCount: number,
...
export function renderTaskList(
  ...
  const { above, below } = scrollIndicators(
    state.scrollOffset,
    heightMode.visibleCount,
    tasks.length,
  );
  ...
  const headerLine =
    ...
    (scrollRight
      ? " ".repeat(
          Math.max(1, width - headerLabel.length - scrollRight.length - 2),
        ) +
        fg("muted", scrollRight) +
        "  "
      : "");
  ...
  const visibleTasks = tasks.slice(
    state.scrollOffset,
    state.scrollOffset + heightMode.visibleCount,
  );
  ...
  for (let i = 0; i < visibleTasks.length; i++) {
    ...
    const taskText = fg(
      textColor,
      trunc(displayText, Math.max(0, maxTextLen), "\u2026"),
    );
