...
export function loadReviewState(cwd: string, change: string, maxRounds = 3, sessionHash?: string): ReviewState {
  const ledger = new ReviewLedger(cwd, sessionHash);
  try { return ledger.load(change, maxRounds); } finally { ledger.close(); }
}

export function saveReviewState(cwd: string, state: ReviewState, sessionHash?: string): void {
  const ledger = new ReviewLedger(cwd, sessionHash);
  try { ledger.save(state); } finally { ledger.close(); }
}

export function processReviewReceipt(cwd: string, receipt: ReviewReceipt, maxRounds = 3, sessionHash?: string): ReviewReceiptResult {
...
export function hasPendingReviewDispatch(cwd: string, change: string, key: string, sessionHash?: string): boolean {
  const ledger = new ReviewLedger(cwd, sessionHash);
  try { return ledger.pendingDispatch(change, key); } finally { ledger.close(); }
}

export function markReviewDispatch(cwd: string, change: string, key: string, sessionHash?: string): void {
  const ledger = new ReviewLedger(cwd, sessionHash);
  try { ledger.markDispatched(change, key); } finally { ledger.close(); }
}

export function hasDurableReviewPass(cwd: string, change: string, sessionHash?: string): boolean {
  const state = loadReviewState(cwd, change, undefined, sessionHash);
