...
export function renderInteractiveView(
  ...
  for (let i = 0; i < doc.items.length; i++) {
    ...
    if (purpose === "questions" && answers && isQuestionItem(item)) {
      ...
      const answerLine = renderAnswerField(
        answer,
        defaultVal,
        contentWidth,
        isCursor,
  ...
  if (defaultVal) {
    ...
    const label = isCursorOnQuestion
      ? theme.fg("dim", "→ ") +
        ...
        theme.fg("accent", "Enter to answer")
...
export function getFooterActions(
  ...
  actions.push({ key: "e", label: "Edit" }, { key: "a", label: "Add" });
...
export function renderFooter(
  ...
  const parts = actions.map(
    (a) => theme.fg("accent", `[${a.key}]`) + " " + theme.fg("dim", a.label),
  );
