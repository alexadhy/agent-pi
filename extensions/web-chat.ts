  ...
  onToolStart(event: ToolExecutionStartEvent): void {
    ...
    if (name === "subagent_create" || name === "subagent_create_batch") {
      ...
      if (name === "subagent_create_batch" && args?.agents) {
        ...
        const names = args.agents
          .map((a: any) => a.name || a.summary || "agent")
          .join(", ");
        ...
        broadcastWS(this.clients, "subagent_start", {
          count: 1,
          names: agentName,
        });
      }
  ...
  return new Promise((resolve) => {
    ...
    const server = createServer((req: IncomingMessage, res: ServerResponse) => {
        ...
        req.on("end", () => {
          try {
            ...
            if (String(data.pin) === pin) {
              ...
              res.setHeader(
                "Set-Cookie",
                `pi_token=${token}; Path=/; HttpOnly; SameSite=Strict`,
              );
      ...
      if (req.method === "GET" && url.pathname === "/") {
        ...
        const html = generateWebChatHTML({
    ...
    wss.on("connection", (ws) => {
      ...
      if (bridge.getTerminalHistory().length === 0) {
        sendWS(client, "terminal_output", {
          line: "[info] Connected — activity will appear here",
        });
      }
...
const ShowChatParams = Type.Object({
  port: Type.Optional(
    Type.Number({
      description: "Specific port to use (default: auto-assigned)",
    }),
  ),
});
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("chat", {
    description:
      "Open web chat (relay mode). '/chat --remote' for tunnel, '/chat stop' to shut down",
    handler: async (args, ctx) => {
      ...
      const remote =
        trimmed === "--remote" || trimmed === "-r" || trimmed === "remote";

      try {
        const { localUrl, lanUrl, pin, tunnelUrl } = await launchChat(
          ctx,
          remote,
        );
        openBrowser(localUrl);
