...
const CycleParams = Type.Object({
  instructions: Type.Optional(
    Type.String({
      description: "Custom instructions for what to focus on in the summary",
    }),
  ),
});
...
function renderCompactionCard(message: any, _options: any, theme: any) {
  ...
  const pctColor =
    percent <= 30 ? "success" : percent <= 60 ? "muted" : "warning";
  const pctText =
    theme.fg(pctColor as any, `${percent}%`) + theme.fg("dim", " context used");
  ...
  if (details?.task) {
    const truncated =
      details.task.length > 72
        ? details.task.slice(0, 69) + "..."
        : details.task;
    detailLines.push(theme.fg("dim", "task ") + theme.fg("muted", truncated));
  }
  ...
  if (details?.recentFiles?.length) {
    ...
    const more =
      details.recentFiles.length > 3
        ? theme.fg("dim", ` +${details.recentFiles.length - 3}`)
        : "";
    detailLines.push(
      theme.fg("dim", "files ") +
        theme.fg("muted", names.join(theme.fg("dim", " / "))) +
        more,
    );
  }

  // ── Assemble card body ──────────────────────────────────────
  const lines: string[] = [title, pctText];
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerMessageRenderer<CompactionCardDetails>(
    "memory-cycle-resume",
  ...
  pi.on("session_before_compact", async (event, ctx) => {
    ...
    try {
      ...
      for (const f of branchOps.read) {
        if (!readFiles.includes(f)) readFiles.push(f);
      }
      for (const f of branchOps.modified) {
  ...
  pi.registerCommand("cycle", {
    description:
      "Compact → new session → restore: fresh context with full memory",
    handler: async (args, ctx) => {
      ...
      if (entries.length < 3) {
        ctx.ui.notify(
          "Session too short to cycle — nothing to compact.",
          "warning",
        );
        return;
      }
      ...
      if (!compactionSummary) {
        ctx.ui.notify(
          "Memory Cycle aborted — compaction produced no summary.",
          "error",
        );
        return;
      }
      ...
      ctx.ui.notify(
        "Memory Cycle: Step 2/3 — Creating fresh session...",
        "info",
      );
      ...
      if (result.cancelled) {
        ctx.ui.notify(
          "Memory Cycle cancelled — session switch was blocked.",
          "warning",
        );
        return;
      }
      ...
      ctx.ui.notify(
        "Memory Cycle complete — fresh context with full memory.",
        "success",
      );
    },
  });
  ...
  pi.on("agent_end", async (_event, ctx) => {
    ...
    ctx.compact({
      ...
      onComplete: () => {
        ...
        const postUsage = ctx.getContextUsage();
        const postPercent = postUsage?.percent
          ? Math.round(postUsage.percent)
          : 0;
      ...
      onError: (err: Error) => {
        ...
        ctx.ui.setStatus("memory-cycle", undefined);
        ctx.ui.notify(
          `Memory Cycle failed: ${err.message}. Try /compact manually.`,
          "error",
        );
      },
    });
  ...
  pi.registerTool({
    ...
    label: "Cycle Memory",
    description:
      "Compact current session, start fresh, and restore memory. Use when context is getting large or you want a clean slate while keeping all progress.",
    ...
    renderResult(result, _options, theme) {
      ...
      const msg =
        status === "scheduled"
          ? theme.fg(
              "dim",
              "Memory cycle scheduled — compacting after this turn",
            )
