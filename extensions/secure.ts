...
export default function secure(pi: ExtensionAPI) {
  ...
  pi.registerCommand("secure", {
    description:
      "AI Security — sweep for vulnerabilities, install protections [sweep|install|status|report]",
...
async function handleStatus(cwd: string, ctx: any) {
  try {
    ...
    checks.push({
      ...
      detail: profile.hasGitIgnore
        ? "Present"
        : "MISSING — secrets may be committed!",
    });
    ...
    checks.push({
      ...
      detail: envIgnored
        ? "Properly ignored"
        : ".env NOT in .gitignore — keys may leak!",
    });

    // Security guard presence
    const hasGuard =
      existsSync(join(cwd, "lib", "security", "ai-security-guard.ts")) ||
      ...
      if (existsSync(pkgPath)) {
        try {
          ...
          hasRateLimit = !!(
            ...
            deps["rate-limiter-flexible"] ||
            deps["bottleneck"] ||
            deps["@upstash/ratelimit"]
          );
        } catch {}
      }
      checks.push({
        ...
        detail: hasRateLimit
          ? "Library detected"
          : "No rate limiting library found",
      });
    }
    ...
    if (lastSweepResult) {
      lines.push(``);
      lines.push(
        `Last sweep: ${lastSweepResult.timestamp} — Score: ${lastSweepResult.score}/100, ${lastSweepResult.findings.length} findings`,
      );
    }
