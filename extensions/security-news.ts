    ...
    .map((item) => {
      ...
      const summary = [
        ...
        normalizeText(item.requiredAction)
          ? `Required action: ${normalizeText(item.requiredAction)}`
          : "",
      ]
        .filter(Boolean)
        .join(" | ");
...
async function fetchNvdByCve(cveId: string): Promise<SecurityNewsItem[]> {
  const data = await fetchJson(
    `${NVD_API_URL}?cveId=${encodeURIComponent(cveId)}`,
  );
  const vulns = safeArray<any>(data?.vulnerabilities);
...
const SOURCES: SecuritySource[] = [
  ...
  },
  {
    ...
    category: "non-profit",
    description:
      "OWASP guidance and project advisories relevant to application and network security.",
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    label: "Security News",
    description:
      "Curated security news and advisory retrieval from trusted sources such as CISA, NVD, OWASP, and CVE. Supports source listing, latest advisories, filtered search, and CVE lookup.",
    parameters: Type.Object({
      action: Type.String({
        description: "Action to perform: sources, latest, search, cve_lookup",
      }),
      query: Type.Optional(
        Type.String({
          description: "Optional search filter for latest/search actions",
        }),
      ),
      source: Type.Optional(
        Type.String({
          description: "Optional source filter: cisa, owasp, nvd, cve",
        }),
      ),
      cve_id: Type.Optional(
        Type.String({ description: "Specific CVE ID for cve_lookup action" }),
      ),
      limit: Type.Optional(
        Type.Number({
    ...
    async execute(_toolCallId, params) {
      ...
      const limit =
        typeof (params as any).limit === "number"
          ? Math.max(1, Math.min(25, (params as any).limit))
          : 10;
      ...
      if (action === "sources") {
        const text = [
          "Trusted security news/advisory sources:",
          "",
          ...SOURCES.map(formatSource),
        ].join("\n");
        return {
          content: [{ type: "text" as const, text }],
          details: { action, count: SOURCES.length },
        };
      }
      ...
      if (sourceId && selectedSources.length === 0) {
        return {
          content: [
            { type: "text" as const, text: `Unknown source: ${sourceId}` },
          ],
          details: { error: "invalid_source" },
        };
      }
      ...
      try {
          ...
          if (!/^CVE-\d{4}-\d{4,7}$/i.test(cveId)) {
            return {
              content: [
                {
                  type: "text" as const,
                  text: "cve_lookup requires a valid CVE ID like CVE-2024-12345.",
                },
              ],
              details: { error: "invalid_cve" },
            };
          }
        ...
        if (items.length === 0) {
          return {
            content: [
              {
                type: "text" as const,
                text: "No trusted security news results matched the request.",
              },
            ],
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `security_news failed: ${error.message}`,
            },
          ],
    ...
    renderCall(args, theme) {
      ...
      return new Text(
        theme.fg("toolTitle", theme.bold("security_news ")) +
    ...
    renderResult(result, _options, theme) {
      ...
      if (details?.error)
        return new Text(
          theme.fg("error", `security_news error: ${details.error}`),
          0,
          0,
        );
      return new Text(
        theme.fg("success", `security_news ${details?.count ?? 0} result(s)`),
        0,
        0,
      );
    },
