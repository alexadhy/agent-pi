...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("thinking", {
    ...
    handler: async (args, ctx) => {
      ...
      if (arg) {
        ctx.ui.notify(
          `Unknown level: ${arg}. Valid: ${THINKING_LEVELS.join(", ")}`,
          "error",
        );
        return;
      }
  ...
  pi.registerCommand("mode", {
    ...
    handler: async (args, ctx) => {
      ...
      if (arg) {
        ctx.ui.notify(
          `Unknown mode: ${arg}. Valid: ${MODES.join(", ")}`,
          "error",
        );
        return;
      }
  ...
  pi.registerTool({
    ...
    label: "Set Mode",
    description:
      "Switch the operational mode. Call this from NORMAL mode to activate PLAN, SPEC, TEAM, CHAIN, or PIPELINE based on task classification.",
    parameters: Type.Object({
      mode: Type.String({
        description:
          "Target mode: NORMAL, PLAN, SPEC, PIPELINE, TEAM, or CHAIN",
      }),
      reason: Type.Optional(
        Type.String({ description: "Why this mode was chosen" }),
      ),
    }),

    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      const { mode: target, reason } = params as {
        mode: string;
        reason?: string;
      };
      ...
      if (!MODES.includes(upper as Mode)) {
        return {
          content: [
            {
              type: "text",
              text: `Unknown mode: ${target}. Valid: ${MODES.join(", ")}`,
            },
          ],
          details: { error: true },
        };
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    (globalThis as any).__piRefreshModeBlock = () => refreshModeBlock(ctx);
    try {
      writeFileSync(MODE_FILE, "NORMAL", "utf-8");
    } catch {}
    if (ctx.hasUI) {
