...
const AskUserParams = Type.Object({
    ...
    Type.Array(
      Type.Object({
        ...
        markdown: Type.Optional(
          Type.String({
            description:
              "Markdown preview shown when this option is highlighted",
          }),
        ),
      }),
  ...
  placeholder: Type.Optional(
    Type.String({ description: "Placeholder text for input mode" }),
  ),
  detail: Type.Optional(
    Type.String({ description: "Detail text for confirm mode" }),
  ),
});
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (mode === "select") {
        if (!options || options.length === 0) {
          return {
            content: [
              {
                type: "text" as const,
                text: "Error: options[] required for select mode",
              },
            ],
          };
        }
        ...
        return {
          content: [
            { type: "text" as const, text: `User selected: ${result}` },
          ],
      ...
      if (mode === "confirm") {
        const confirmed = await ctx.ui.confirm(question, detail || "", {
          timeout: 60000,
        });
        return {
          content: [
      ...
      return {
        content: [
          { type: "text" as const, text: `Error: unknown mode '${mode}'` },
        ],
      };
    },
