...
import {
  ...
  type SpecDocument,
} from "./lib/spec-viewer-html.ts";
import {
  createSpecStandaloneExport,
  loadVisualAsExportAsset,
  saveStandaloneExport,
  type SpecExportDocument,
} from "./lib/viewer-standalone-export.ts";
...
function buildStandaloneSpecDocuments(
  folderPath: string,
  ...
  return documents.map((doc) => {
    if (doc.isVisuals) {
      return {
        ...
        isVisuals: true,
        visuals: (doc.visualFiles || []).map((file) =>
          loadVisualAsExportAsset(folderPath, file),
        ),
      };
    }
...
function startSpecViewerServer(
  ...
  port: number;
  server: Server;
  waitForResult: () => Promise<SpecViewerResult>;
}> {
    ...
    const server = createServer((req: IncomingMessage, res: ServerResponse) => {
      ...
      if (req.method === "POST" && url.pathname === "/save") {
        ...
        req.on("end", () => {
          try {
            ...
            writeFileSync(
              commentsPath,
              JSON.stringify({ comments: data.comments || [] }, null, 2),
              "utf-8",
            );
            res.writeHead(200, { "Content-Type": "application/json" });
            res.end(JSON.stringify({ ok: true }));
      ...
      if (req.method === "POST" && url.pathname === "/export-standalone") {
        ...
        req.on("end", () => {
          try {
            const data = JSON.parse(body || "{}");
            const exportDocs = buildStandaloneSpecDocuments(
              folderPath,
              documents,
              data.markdownChanges || {},
            );
            const html = createSpecStandaloneExport({
              title,
              documents: exportDocs,
            });
            const saved = saveStandaloneExport({
              filePrefix: "spec-readonly",
              html,
            });
            res.writeHead(200, { "Content-Type": "application/json" });
            res.end(
              JSON.stringify({
                ok: true,
                message: `Standalone export saved to ~/Desktop/${saved.fileName}`,
              }),
            );
          } catch (err: any) {
            res.writeHead(500, { "Content-Type": "application/json" });
...
const ShowSpecParams = Type.Object({
  folder_path: Type.String({
    description:
      "Path to the spec folder (e.g. context-os/specs/2025-06-25-feature/)",
  }),
  title: Type.Optional(
    Type.String({ description: "Title to display in the viewer header" }),
  ),
});
...
export default function (pi: ExtensionAPI) {
    ...
    try {
      ...
      if (result.comments && result.comments.length > 0) {
        try {
          writeFileSync(
            commentsPath,
            JSON.stringify({ comments: result.comments }, null, 2),
            "utf-8",
          );
        } catch {}
      }
      ...
      try {
        const editedDocCount = result.markdownChanges
          ? Object.keys(result.markdownChanges).length
          : 0;
      ...
      if (!existsSync(folderPath) || !statSync(folderPath).isDirectory()) {
        return {
          content: [
            {
              type: "text" as const,
              text: `Error: folder not found: ${folder_path}`,
            },
          ],
        };
      }
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Spec viewer error: ${err.message}`,
            },
          ],
        };
      }
      ...
      if (details.action === "changes_requested") {
        ...
        return new Text(
          outputLine(
            theme,
            "warning",
            `Changes requested (${count} comment${count !== 1 ? "s" : ""})`,
          ),
          0,
          0,
        );
  ...
  pi.registerCommand("spec", {
    description:
      "Open the spec viewer for a spec folder (e.g. /spec context-os/specs/2025-06-25-feature/)",
