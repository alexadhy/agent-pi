...
const DEFAULT_PORTS =
  "22,53,80,123,135,139,443,445,3000,3389,5000,8000,8080,8443";
...
function validateTarget(target: string): { ok: boolean; reason?: string } {
  if (!target || /\s/.test(target))
    return {
      ok: false,
      reason: "Target is required and must not contain whitespace.",
    };
  if (/[a-z]/i.test(target) && net.isIP(target) === 0) {
    return {
      ok: false,
      reason:
        "Only literal IP addresses are allowed. Hostnames and domains are refused for safety.",
    };
  }
  ...
  return {
    ok: false,
    reason: "Target must be loopback or a private local-network IP address.",
  };
}
...
function validatePorts(ports: string): { ok: boolean; reason?: string } {
  ...
  if (!/^\d+(,\d+)*$/.test(ports)) {
    return {
      ok: false,
      reason: "Ports must be a comma-separated allowlist like 22,80,443.",
    };
  }
  ...
  if (values.length > 25)
    return {
      ok: false,
      reason: "Too many ports requested. Maximum 25 ports per safe scan.",
    };
  if (values.some((p) => !Number.isInteger(p) || p < 1 || p > 65535)) {
    return {
      ok: false,
      reason: "Ports must be valid integers between 1 and 65535.",
    };
  }
  return { ok: true };
}
...
function parseGNmap(
  stdout: string,
): Array<{ host: string; openPorts: string[] }> {
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    description:
      "Safe, low-impact local port analysis using a guarded nmap wrapper. Only loopback and private IP targets are allowed. Aggressive flags, public targets, hostnames, and arbitrary options are refused.",
    parameters: Type.Object({
      target: Type.String({
        description: "Literal loopback or private IP address to scan.",
      }),
      ports: Type.Optional(
        Type.String({
          description:
            "Comma-separated allowlist of ports. Defaults to a small common set.",
        }),
      ),
      dry_run: Type.Optional(
        Type.Boolean({
          description:
            "If true, return the bounded command template without executing it.",
        }),
      ),
    }),
    async execute(_toolCallId, params) {
      const target =
        typeof (params as any).target === "string"
          ? (params as any).target.trim()
          : "";
      const ports =
        typeof (params as any).ports === "string"
          ? (params as any).ports.trim()
          : DEFAULT_PORTS;
      ...
      if (!targetCheck.ok) {
        return {
          content: [
            { type: "text" as const, text: `Refused: ${targetCheck.reason}` },
          ],
      ...
      if (!portCheck.ok) {
        return {
          content: [
            { type: "text" as const, text: `Refused: ${portCheck.reason}` },
          ],
      ...
      if (dryRun) {
        return {
          content: [
            {
              type: "text" as const,
              text: `Dry run only. Safe command template:\n\n${commandPreview}`,
            },
          ],
      ...
      try {
        ...
        const summary =
          parsed.length === 0
            ...
            : parsed
                .map(
                  (entry) =>
                    `- ${entry.host}: ${entry.openPorts.length ? entry.openPorts.join(", ") : "no open ports reported"}`,
                )
                .join("\n");
        return {
          content: [
            {
              type: "text" as const,
              text: `Safe port scan complete for ${target}.\n\n${summary}`,
            },
          ],
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `safe_port_scan failed: ${error.message}`,
            },
          ],
    ...
    renderCall(args, theme) {
      const p = args as any;
      return new Text(
        theme.fg("toolTitle", theme.bold("safe_port_scan ")) +
          theme.fg("accent", p.target || ""),
        0,
        0,
      );
    },
    renderResult(result, _options, theme) {
      ...
      if (details?.error)
        return new Text(
          theme.fg("error", `safe_port_scan error: ${details.error}`),
          0,
          0,
        );
      if (details?.dryRun)
        return new Text(theme.fg("accent", "safe_port_scan dry run"), 0, 0);
