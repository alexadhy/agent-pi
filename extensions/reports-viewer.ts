...
function openOriginalReport(entry: any): void {
  ...
  if (process.platform === "darwin") {
    if (entry.category === "spec") {
      execSync(
        `open -na Terminal --args bash -lc ${quoteArg(`cd ${quoteArg(process.cwd())} && pi /spec ${quoteArg(path)}`)}`,
        { stdio: "ignore", shell: true },
      );
    } else {
      execSync(
        `open -na Terminal --args bash -lc ${quoteArg(`cd ${quoteArg(process.cwd())} && pi /show-file ${quoteArg(path)}`)}`,
        { stdio: "ignore", shell: true },
      );
    }
    return;
  ...
  if (entry.category === "spec") {
    execSync(
      `bash -lc ${quoteArg(`cd ${quoteArg(process.cwd())} && pi /spec ${quoteArg(path)} >/dev/null 2>&1 &`)}`,
      { stdio: "ignore", shell: true },
    );
  } else {
    execSync(
      `bash -lc ${quoteArg(`cd ${quoteArg(process.cwd())} && pi /show-file ${quoteArg(path)} >/dev/null 2>&1 &`)}`,
      { stdio: "ignore", shell: true },
    );
  }
}
    ...
    const server = createServer((req: IncomingMessage, res: ServerResponse) => {
      ...
      if (req.method === "GET" && url.pathname === "/") {
        ...
        const html = generateReportsViewerHTML({
          title,
          port,
          entries: loadReportIndex().entries,
        });
      ...
      if (req.method === "POST" && url.pathname === "/open") {
        ...
        req.on("end", () => {
          try {
            ...
            const entry = loadReportIndex().entries.find(
...
const ShowReportsParams = Type.Object({
  title: Type.Optional(
    Type.String({ description: "Title for the reports browser view" }),
  ),
});
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    label: "Show Reports",
    description:
      "Open a searchable /reports browser view for persisted plans, questions, specs, and completion reports.",
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      return {
        content: [{ type: "text" as const, text: "Reports viewer closed." }],
      };
    },
    renderCall(args, theme) {
      const text =
        theme.fg("toolTitle", theme.bold("show_reports ")) +
        theme.fg("accent", (args as any).title || "Reports Index");
