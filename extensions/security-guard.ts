...
function extractStrings(obj: any, depth = 0): string[] {
  ...
  if (Array.isArray(obj))
    return obj.flatMap((v) => extractStrings(v, depth + 1));
...
export default function securityGuard(pi: ExtensionAPI) {
  ...
  pi.registerMessageRenderer<GuardCardDetails>(
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    const toolBudgetSettings: ToolBudget | null =
      s.tool_budget_max_tool_calls_per_turn != null
        ? {
            max_tool_calls_per_turn:
              s.tool_budget_max_tool_calls_per_turn ?? 200,
            max_tool_calls_per_session:
              s.tool_budget_max_tool_calls_per_session ?? 2000,
            max_bash_calls_per_turn:
              s.tool_budget_max_bash_calls_per_turn ?? 100,
            warn_threshold_pct: s.tool_budget_warn_threshold_pct ?? 0.8,
          }
        : null;
    if (toolBudgetSettings) {
      const budgetResult = checkToolBudget(
        toolName,
        budgetCounters,
        toolBudgetSettings,
      );
      if (budgetResult) {
        ...
        if (budgetResult.severity === "block") {
          ...
          return {
            block: true,
            reason: formatThreatsForBlock(
              [budgetResult],
              policy.settings.verbose_blocks,
            ),
          };
        }
        stats.warned++;
    ...
    for (const t of warnThreats) {
      ...
      if (ctx?.ui?.notify) {
        ctx.ui.notify(
          `⚠️ Security: ${t.description} — ${truncate(t.matched, 60)}`,
          "warning",
        );
      }
    }
    ...
    if (blockThreats.length > 0) {
      ...
      const reason = formatThreatsForBlock(
        blockThreats,
        policy.settings.verbose_blocks,
      );
  ...
  pi.on("context", async (event, ctx) => {
    ...
    const maxResultChars =
      (policy.settings as any).max_tool_result_chars ?? 100000;
    ...
    const repairedMessages = messages.map((msg: any) => {
      ...
      const newContent = currentContent.map((block: any) => {
        ...
        if (cleaned !== block.text) {
          ...
          emitGuardCard(
            `stripped ${redactions.length} injection(s)`,
            toolLabel,
          );
    ...
    if (
      ...
      (policy.settings as any).detect_prompt_leakage !== false
    ) {
      for (let i = 0; i < repairedMessages.length; i++) {
        ...
        if (leakage) {
          ...
          repairedMessages[i] = {
            ...msg,
            content:
              "[System prompt leakage detected and blocked. The assistant attempted to reveal its system instructions.]",
          };
        }
      }
    }
      ...
      for (let i = 0; i < repairedMessages.length; i++) {
        ...
        if (typeof content === "string") {
          ...
          if (result.found) {
            ...
            emitGuardCard(
              `redacted ${result.matchCount} secret(s)`,
              "assistant output",
            );
            audit.log({
              timestamp: now(),
    ...
    handler: async (args, ctx) => {
      ...
      switch (subcommand) {
        ...
        case "log": {
          ...
          if (entries.length === 0) {
            ctx.ui.notify(
              "🛡️ Security audit log is empty — no threats detected.",
              "info",
            );
          } else {
            ctx.ui.notify(
              `🛡️ Recent audit log (last ${entries.length}):\n\n${entries.join("\n")}`,
              "info",
            );
          }
          break;
        ...
        case "policy": {
          const summary = [
            ...
            policy.blocked_commands.length > 8
              ? `  ... and ${policy.blocked_commands.length - 8} more`
              : "",
            ``,
            `Protected paths (${policy.protected_paths.length}):`,
  ...
  function updateStatusBar(ctx: any) {
    ...
    if (total > 0) {
      ctx.ui.setStatus(
        "security",
        `🛡️ Security (${stats.blocked}🛑 ${stats.warned}⚠️)`,
      );
    } else {
      ctx.ui.setStatus("security", "🛡️ Security Guard");
    }
