...
export default function (pi: ExtensionAPI) {
        ...
        spawnToolkitWorker(
          {
            ...
            systemPrompt: [
              ...
              ...systemPromptArgs.filter((_, i) => i % 2 === 1),
            ]
              .filter(Boolean)
              .join("\n\n"),
          },
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      commanderTaskId: Type.Optional(
        Type.Number({
          description:
            "Pre-assigned Commander task ID (avoids race conditions)",
        }),
      ),
      ...
      timeout: Type.Optional(
        Type.Number({
          description:
            "Max runtime in milliseconds. Defaults by role: scout=10min, builder=30min, reviewer=15min, default=20min. Set 0 to disable.",
        }),
      ),
    }),
  ...
  pi.registerTool({
    ...
    execute: async (callId, args, _signal, _onUpdate, ctx) => {
      ...
      if (!defs || defs.length === 0) {
        return {
          content: [{ type: "text", text: "Error: No agents specified." }],
        };
      }
      ...
      return {
        content: [
          {
            type: "text",
            text: `Batch spawned ${states.length} subagents: ${ids}`,
          },
        ],
      };
    },
  ...
  pi.registerTool({
    ...
    execute: async (callId, args, _signal, _onUpdate, ctx) => {
      ...
      if (state.status === "running") {
        return {
          content: [
            { type: "text", text: `Error: SA${args.id} is still running.` },
          ],
        };
      }
  ...
  pi.registerTool({
    ...
    execute: async () => {
      ...
      const list = Array.from(agents.values())
        .map(
          (s) => `SA${s.id} [${s.status.toUpperCase()}] ${s.name} - ${s.task}`,
        )
        .join("\n");
  ...
  function preSpawnScout(ctx: any) {
    ...
    spawnAgent(
      state,
      "You are now on standby. Respond with exactly: Ready.",
      ctx,
    );
  }
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    modelsConfig = loadAgentModelsConfig(
      ctx.cwd || process.cwd(),
      extProjectDir,
    );
    const standardAgents = scanAgentDefs(
      ctx.cwd || process.cwd(),
      extProjectDir,
      modelsConfig,
    );
    const toolkitModelsConfig = loadToolkitModelsConfig(
      ctx.cwd || process.cwd(),
      extProjectDir,
    );
