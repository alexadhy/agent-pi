...
function generateTape(
  ...
  switch (command) {
    ...
    case "custom":
      tape = scenarioCustom(
        args || "echo 'No commands specified'",
        relCaptureDir,
        ts,
        opts,
      );
...
function formatResult(result: CaptureResult): string {
  ...
  if (result.screenshots.length > 0) {
    ...
    lines.push("");
    lines.push(
      "Use Read on any screenshot path above to view the captured UI.",
    );
  } else {
          ...
          s === "tasks"
            ...
            : s === "modes"
              ...
              : s === "footer"
                ? "footer — Footer status bar"
                : s === "theme"
                  ? "theme <name> — Pi with a specific VHS theme"
                  : s === "pi"
                    ? "pi <prompt> — Run Pi with a prompt and capture output"
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      scenario: Type.String({
        description:
          "Capture scenario: tasks, modes, footer, theme <name>, pi <prompt>, custom <cmds>",
      }),
      width: Type.Optional(
        Type.Number({
          description: "Terminal width in pixels (default: 1400)",
        }),
      ),
      height: Type.Optional(
        Type.Number({
          description: "Terminal height in pixels (default: 900)",
        }),
      ),
      fontSize: Type.Optional(
        Type.Number({ description: "Font size in pixels (default: 13)" }),
      ),
      theme: Type.Optional(
        Type.String({ description: "VHS terminal theme (default: Dracula)" }),
      ),
    }),

    async execute(_toolCallId, params, _signal, onUpdate, ctx) {
      const { scenario, width, height, fontSize, theme } = params as {
        scenario: string;
        width?: number;
        height?: number;
        fontSize?: number;
    ...
    renderCall(_params, _theme) {
      ...
      const RST = "\x1b[0m";
      return new Text(
        `${DIM}debug-capture:${RST} ${BRIGHT}${p.scenario}${RST}`,
        0,
        0,
      );
    },
