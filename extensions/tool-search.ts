...
const ToolSearchParams = Type.Object({
  ...
  query: Type.Optional(
    Type.String({
      description:
        "Search query — matches tool names, descriptions, tags, and categories",
    }),
  ),
  category: Type.Optional(
    Type.String({
      description:
        "Filter by category (for 'list' operation). Use 'list' without category to see all categories.",
    }),
  ),
  tool_name: Type.Optional(
    Type.String({
      description: "Tool name to inspect (for 'inspect' operation)",
    }),
  ),
});
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
        ...
        if (!query) {
          return {
            content: [
              {
                type: "text" as const,
                text: "Error: 'query' is required for search operation",
              },
            ],
          };
        }
          ...
          if (tools.length === 0) {
            return {
              content: [
                {
                  type: "text" as const,
                  text: `No tools in category "${category}". Use list without category to see available categories.`,
                },
              ],
        ...
        if (!tool_name) {
          return {
            content: [
              {
                type: "text" as const,
                text: "Error: 'tool_name' is required for inspect operation",
              },
            ],
          };
        }
        ...
        if (!entry) {
          ...
          return {
            content: [
              {
                type: "text" as const,
                text: `Tool "${tool_name}" not found.${suggestion}`,
              },
            ],
      ...
      return {
        content: [
          {
            type: "text" as const,
            text: `Unknown operation: ${operation}. Use 'search', 'list', or 'inspect'.`,
          },
        ],
      };
    },
    ...
    renderResult(result, { expanded }, theme) {
      ...
      if (details.operation === "search" || details.operation === "list") {
        ...
        if (details.category)
          summary += theme.fg("dim", ` in ${details.category}`);
      ...
      if (details.operation === "inspect") {
        ...
        return new Text(
          theme.fg("error", `✗ Tool not found: ${details.tool_name}`),
          0,
          0,
        );
      }
