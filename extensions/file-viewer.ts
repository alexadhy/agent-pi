...
function parseRange(content: string, lineRange?: string): string {
  ...
  const end = match[2]
    ? Math.min(lines.length, parseInt(match[2], 10))
    : start + 1;
  const out: string[] = [];
  if (start > 0) out.push("...");
...
function launchEditor(
  ...
  if (!commandMap[editor])
    return { ok: false, error: `Unsupported editor: ${editor}` };
  try {
...
function startFileViewerServer(opts: {
  ...
  port: number;
  server: Server;
  waitForResult: () => Promise<FileViewerResult>;
}> {
        ...
        req.on("end", () => {
          try {
            ...
            res.end(
              JSON.stringify({
                ok: false,
                error: err?.message || "Editor launch failed",
              }),
            );
          }
        });
...
const ShowFileParams = Type.Object({
  ...
  title: Type.Optional(
    Type.String({ description: "Optional title shown in the viewer header" }),
  ),
  line_range: Type.Optional(
    Type.String({ description: "Optional line range like '45-60' or '45'" }),
  ),
  editable: Type.Optional(
    Type.Boolean({
      description: "Whether to allow editing and saving from the browser UI",
    }),
  ),
});
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    label: "Close Viewer",
    description:
      "Close the currently active local browser viewer from the CLI if one is open.",
    ...
    async execute() {
      ...
      if (!closed.closed) {
        return {
          content: [
            { type: "text" as const, text: "No active local viewer is open." },
          ],
        };
      }
      return {
        content: [
          {
            type: "text" as const,
            text: `Closed ${closed.kind} viewer${closed.title ? `: ${closed.title}` : ""}.`,
          },
        ],
      };
    },
  ...
  pi.registerCommand("show-file-help", {
    ...
    handler: async (_args, ctx) => {
      outputLine(
        ctx,
        'show_file { file_path: "path/to/file", editable: true }',
        "info",
      );
    },
  });
